...
/*public:
	virtual */void MergeSort(vector<string>& data)
	{
		//break everything into singletons
		queue<vector<string>> data_bits{};
		for (auto& item : data)
		{
			vector<string> singleton{ item };
			data_bits.push(singleton);
		}

		while (data_bits.size() > 1)
		{
			vector<string> first = data_bits.front();
			data_bits.pop();
			vector<string> second = data_bits.front();
			data_bits.pop();
